        ...
        sectionTabs = {
            ...
            },
            {
                ...
                sections = {
                    ...
                    { name = "Display", controls = { "button:100%", "button:105%", "button:110%" } }
                }
            },
            {
                name = "Others",
...
local function create(className, props)
    ...
    if instance:IsA("GuiObject") and props.BorderSizePixel == nil then
        instance.BorderSizePixel = 0
    end
    if instance:IsA("GuiObject") and props.BackgroundTransparency == nil and props.BackgroundColor3 == nil then
    ...
    if (className == "ImageLabel" or className == "ImageButton") and props.ScaleType == nil then
        instance.ScaleType = Enum.ScaleType.Fit
    end
    if (className == "TextLabel" or className == "TextButton") and props.RichText == nil then
    ...
    if (className == "TextLabel" or className == "TextBox") and props.TextXAlignment == nil then
...
local function addRoundedStroke(instance, radius, color, thickness, transparency)
    addCorner(instance, radius)
    return addStroke(instance, color, thickness, transparency)
end

local function addHueGradient(parent, name)
...
local function getBlendSequence(baseColor)
...
local function addBlendGradient(parent, name, baseColor)
    return create("UIGradient", {
        Name = name,
        Color = getBlendSequence(baseColor),
        Parent = parent
    })
end

local function updateBlendGradient(parent, name, baseColor)
    local gradient = parent and parent:FindFirstChild(name)
    if gradient then
        gradient.Color = getBlendSequence(baseColor)
    end
end

local function createPickerSelector(parent, height, position, visible)
    local selector = create("Frame", {
        Size = UDim2.fromOffset(6, height or 20),
    ...
    addRoundedStroke(selector, UDim.new(1, 0), theme.background)
    return selector
end

local function createPickerBar(parent, size, position, visible)
...
local function rebuildSearchIndex()
    ...
    for _, tab in ipairs(tabs) do
        ...
        if tab.sectionTabs then
            sections = {}
            for _, group in ipairs(tab.sectionTabs) do
...
local buttonHandlers = {
    ...
    ["100%"] = function() ui.menuScale.Scale = 1 end,
    ["105%"] = function() ui.menuScale.Scale = 1.05 end,
    ["110%"] = function() ui.menuScale.Scale = 1.1 end,
...
local function registerControlLabel(tabName, label, originalText)
    controlLabelsByTab[tabName] = controlLabelsByTab[tabName] or {}
    controlOriginalText[tabName] = controlOriginalText[tabName] or {}
    table.insert(controlLabelsByTab[tabName], label)
    table.insert(controlOriginalText[tabName], originalText)
end
...
local function makeControl(parent, spec, currentSection, currentTabName)
    ...
    if controlType == "button" then
		...
		registerControlLabel(currentTabName, buttonFrame, controlName)
		
        addRoundedStroke(buttonFrame, isThemeResetButton and 6 or 8, isThemeResetButton and theme.accent or theme.stroke, 1, isThemeResetButton and 0.3 or 0)
    ...
    elseif controlType == "toggle" then
        ...
        addRoundedStroke(toggle, 4, theme.stroke, 1, 0.2)
		...
		registerControlLabel(currentTabName, toggleLabel, controlName)
        ...
        if isVisualColorToggle then
            ...
            if controlName == "Boxes" then
                ...
                addRoundedStroke(cornerToggle, 4)
            ...
            addRoundedStroke(preview, 4)

            palette = createPickerBar(subRow, UDim2.new(1, -4, 0, 16), UDim2.fromOffset(0, controlName == "Boxes" and 50 or 26), false)

            addHueGradient(palette, "BoxPickerGradient")

            selector = createPickerSelector(palette, 20, UDim2.new(0.5, 0, 0.5, 0), false)

            if not singleBarOnly then
                blendPalette = createPickerBar(subRow, UDim2.new(1, -4, 0, 16), UDim2.fromOffset(0, controlName == "Boxes" and 70 or 46), false)

                addBlendGradient(blendPalette, "BoxBlendGradient", currentBaseColor)

                blendSelector = createPickerSelector(blendPalette, 20, UDim2.new(currentBlend, 0, 0.5, 0), false)
            end
        ...
        if preview and palette and selector then
            ...
            local function refreshBlendGradient()
                ...
                updateBlendGradient(blendPalette, "BoxBlendGradient", currentBaseColor)
            end
    ...
    elseif controlType == "slider" then
		...
		registerControlLabel(currentTabName, valueLabel, controlName)
        ...
        if controlName == "Walk Speed" or controlName == "Fly Speed" or controlName == "Jump Height" then
            ...
            addRoundedStroke(minusButton, 6)
        ...
        local bar = create("Frame", {
            ...
            Position = UDim2.new(0, barXOffset, 0, 25),
            BackgroundColor3 = theme.cardAlt,
            Parent = holder
        })
    ...
    elseif controlType == "colorpicker" then
        ...
        if isThemeEditorColor then
            addRoundedStroke(row, 6, theme.stroke, 1, 0.55)
        end
		...
		registerControlLabel(currentTabName, pickerLabel, controlName)
        ...
        addRoundedStroke(preview, 6)

        local currentBaseColor = getThemeColor(controlName)
        local currentBlend = 0
        local palette = createPickerBar(row, UDim2.new(1, -8, 0, 18), UDim2.fromOffset(8, isThemeEditorColor and 36 or 26), not isThemeEditorColor)

        addHueGradient(palette, "ThemePickerGradient")

        local selector = createPickerSelector(palette, 24, UDim2.new(0.75, 0, 0.5, 0), true)
        ...
        if isThemeEditorColor then
            blendPalette = createPickerBar(row, UDim2.new(1, -8, 0, 18), UDim2.fromOffset(8, 58), false)

            addBlendGradient(blendPalette, "ThemeBlendGradient", currentBaseColor)

            blendSelector = createPickerSelector(blendPalette, 24, UDim2.new(0, 0, 0.5, 0), false)
        end
         
        local function refreshThemeBlendGradient()
            if not blendPalette then
                return
            end

            updateBlendGradient(blendPalette, "ThemeBlendGradient", currentBaseColor)
        end

        local function applyThemePickerColor(silent)
            local color = currentBaseColor
            if isThemeEditorColor and blendPalette then
                color = currentBaseColor:Lerp(Color3.fromRGB(255, 255, 255), currentBlend)
                if currentBlend >= 0.995 then
                    color = Color3.fromRGB(175, 175, 175)
                end
            end
        ...
        local function setColorFromX(mouseX, silent)
            ...
            currentBaseColor = Color3.fromHSV(hue, 0.83, 0.98)
            refreshThemeBlendGradient()
            applyThemePickerColor(silent)
        end
        ...
        if isThemeEditorColor then
            local function setExpanded(expanded)
                ...
                if blendPalette then
                    blendPalette.Visible = expanded
                end
                if blendSelector then
                    blendSelector.Visible = expanded
                end
                selector.Visible = expanded
                row.Size = UDim2.new(1, 0, 0, expanded and 78 or 30)
            end

            preview.MouseButton1Click:Connect(function()
                if activeThemeColorPicker and activeThemeColorPicker.row ~= row then
                    activeThemeColorPicker.palette.Visible = false
                    activeThemeColorPicker.selector.Visible = false
                    if activeThemeColorPicker.blendPalette then
                        activeThemeColorPicker.blendPalette.Visible = false
                    end
                    if activeThemeColorPicker.blendSelector then
                        activeThemeColorPicker.blendSelector.Visible = false
                    end
                    activeThemeColorPicker.row.Size = UDim2.new(1, 0, 0, 30)
                end
                setExpanded(not palette.Visible)
                activeThemeColorPicker = palette.Visible and {
                    row = row,
                    palette = palette,
                    selector = selector,
                    blendPalette = blendPalette,
...
elseif controlType == "dropdown" then
	...
	registerControlLabel(currentTabName, label, controlName)
...
if not customVisualsLayout then
    ...
    local function updatePageCanvas()
        local scale = ui.menuScale and ui.menuScale.Scale or 1
        local extraPadding = math.max(0, math.floor((1 - scale) * 420))
        page.CanvasSize = UDim2.fromOffset(0, pageLayout.AbsoluteContentSize.Y + 14 + extraPadding)
    end
    ...
    ui.menuScale:GetPropertyChangedSignal("Scale"):Connect(updatePageCanvas)
...
(function()
    ...
    local function updateTabBarCanvas()
        local scale = ui.menuScale and ui.menuScale.Scale or 1
        local extraPadding = math.max(0, math.floor((1 - scale) * (ui.tabBar.AbsoluteSize.Y + 40)))
        ui.tabBar.CanvasSize = UDim2.fromOffset(0, tabLayout.AbsoluteContentSize.Y + 12 + extraPadding)
    end

    tabLayout:GetPropertyChangedSignal("AbsoluteContentSize"):Connect(updateTabBarCanvas)
    ui.menuScale:GetPropertyChangedSignal("Scale"):Connect(updateTabBarCanvas)
